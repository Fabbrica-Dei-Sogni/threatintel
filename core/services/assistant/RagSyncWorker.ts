...
export class RagSyncWorker implements ILongRunningService {
    ...
    private async runMaterialization() {
        ...
        this.campaignService.materializeCampaignSummaries().catch(err => 
            this.logger.error(`[${this.serviceName}] Campaign materialization failed: ${err}`)
        );

        // 2. Materializzazione Attacchi IP-centrici
        this.threatLogService.materializeAttackSummaries().catch(err => 
            this.logger.error(`[${this.serviceName}] Attack materialization failed: ${err}`)
        );
    }
}
